#' Make the Covariate Plot
#' @param out the output of the model
#' @export

make_covariate_plot <- function(out){
	...
	data1 = bayesplot::mcmc_intervals_data(first.intervention,prob=.95,
																				 transformation=function(x) 1-exp(-x),
																				 point_est="mean")
	data1$type = "First Intervention"

	data2 = bayesplot::mcmc_intervals_data(alpha[,c(1,2,3,5,6)],
																				 prob = .95,transformation=function(x) 1-exp(-x),
																				 point_est="mean")
	...
	args_outer <- list(mapping = ggplot2::aes_(x = ~ll, xend = ~hh, y = ~parameter,
																		yend = ~parameter)) #, color = bayesplot::get_color("mid"))
	args_inner <- list(mapping = ggplot2::aes_(x = ~l, xend = ~h, y = ~parameter,
																		yend = ~parameter), size = 2, show.legend = FALSE)
	args_point <- list(mapping = ggplot2::aes_(x = ~m, y = ~parameter),
										 data = data, size = 4, shape = 21)

	args_point$color <- "blue" #get_color("dark_highlight")

	point_func <- ggplot2::geom_point
	layer_outer <- do.call(ggplot2::geom_segment, args_outer)
	layer_inner <- do.call(ggplot2::geom_segment, args_inner)
	...
	p = ggplot2::ggplot(data) +
		ggpubr::theme_pubr() +
		ggplot2::geom_point(
			ggplot2::aes(x=m,y=parameter,colour=type),
			position = ggplot2::position_dodge(-.5)) +
		ggplot2::geom_linerange(
			ggplot2::aes(xmin=ll,xmax=hh,y=parameter,colour=type),
									 position = ggplot2::position_dodge(-.5)) +
		ggplot2::scale_x_continuous(breaks=seq(0,1,.25),labels = c("0%\n(no effect on transmissibility)",
																											"25%","50%","75%","100%\n(ends transmissibility)"),
											 expand=c(0.005,0.005),expression(paste("Relative % reduction in  ",R[t])))  +
		ggplot2::scale_colour_manual(name = "", #labels = c("50%", "95%"),
												values = c(("coral4"), ("seagreen"))) +

		ggplot2::geom_vline(xintercept=1,colour="darkgray") +
		ggplot2::scale_y_discrete("Governmental intervention\n") +
		#geom_vline(xintercept=0,colour="darkgray") +
		ggplot2::theme(plot.margin = margin(0, 2, 0, .5, "cm"))
	#+ guides(fill=guide_legend(nrow=2))
	p
	ggplot2::ggsave(filename = "results/covars-alpha-reduction.png",
				 p,height=4,width=8)
