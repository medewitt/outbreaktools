#' Process Covariates for Stan Models
#' @param countries the countries or units to consider
#' @param interventions the data.frame of interventions
#' @param d the data.frame of interventions
#' @param ifr.by.country IFR by country
#' @param N2 number of simulations
#' @param serial_interval the path to serial intervals, use default
#' @param min_date the minimum date to use in the padding
#'
#' @importFrom stats ecdf
#'
#' @export

process_covariates <- function(countries,
															 interventions,
															 d,
															 ifr.by.country,
															 N2,
	...
	if(!is.null(serial_interval)){
		serial.interval = read.csv(serial_interval)
	} else{
		serial.interval <- covid19_serial_interval
	}
	...
	dates <- list()
	reported_cases <- list()
	stan_data <- list(M=length(countries$Regions),
										N=NULL,deaths=NULL,f=NULL,
										N0=6,
										cases=NULL,
	...
	for (Country in countries$Regions){
		...
		date_min <- lubridate::dmy(min_date)
		...
		df_features = data.frame('school' = school,
